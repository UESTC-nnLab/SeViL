_base_ = [
    'mmdet::_base_/default_runtime.py', '../mixpl_itsdt_detection.py'
]
...
model = dict(
    ...
    semi_train_cfg=dict(
        ...
        erase_thr=0.7,
        cls_pseudo_thr=0.4,
        # max_pseudo_bbox_wh = (25, 25),
        freeze_teacher=True,
        sup_weight=1.0,
        unsup_weight=2.0,
        min_pseudo_bbox_wh=(1, 1)),
...
train_cfg = dict(
    type='IterBasedTrainLoop', max_iters=180000, val_interval=1000,val_begin = 1000) #,val_begin = 3000
val_cfg = dict(type='TeacherStudentValLoop') # ValLoop TeacherStudentValLoop
...
default_hooks = dict(
    checkpoint=dict(by_epoch=False, interval=500, max_keep_ckpts=1,save_best='student/coco/bbox_mAP_50',  # 按照该指标保存最优模型
        type='CheckpointHook'),
