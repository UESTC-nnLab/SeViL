...
detector = dict(
    ...
    data_preprocessor=dict(
        type='DetDataPreprocessor',  ###用的Coco的std和mean,可以改成对应数据集的std和mean
    ...
    backbone=dict(
        ...
        depth=50,
        num_stages=4,
        # out_indices=(1, 2, 3), # 5,512,64,64  5 1024 32 32,  5 2048 16 16
        out_indices=(1,2,3),
        frozen_stages=1,
        norm_cfg=dict(type='BN', requires_grad=False),
        norm_eval=True,
        style='pytorch',
        init_cfg=dict(type='Pretrained', checkpoint='torchvision://resnet50')), #resnet50-0676ba61.pth
    neck=dict(
        type='ChannelMapper',
        in_channels=[512,1024,2048],
        kernel_size=1,
        out_channels=256,   # 5,256 64,64  5,256 32,32  5,256 16,16    5,256 8,8
        act_cfg=None,
...
train_dataloader = dict(
    batch_size=4,
    num_workers=0,#5
    persistent_workers=False,###for pdb debug
...
train_cfg = dict(
    type='IterBasedTrainLoop', max_iters=90000, val_interval=1000)
