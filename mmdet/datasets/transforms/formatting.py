...
class PackDetInputs(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        packed_results = dict()
        if 'img' in results:
            # imgs = results['img']  #5个 512, 512, 3
            # imgs = np.stack(imgs, axis=0) # 5, 512, 512, 3
            # imgs = imgs.transpose(0, 3, 1, 2) #(5, 3, 512, 512)
            # packed_results['inputs'] = to_tensor(imgs) #torch.Size([5, 3, 512, 512])  ####原来这么修改的
                # To improve the computational speed by by 3-5 times, apply: 这是源码注释
                ...
                #     img = to_tensor(img)
                # else:
                #     img = to_tensor(img).permute(2, 0, 1).contiguous() #torch.Size([3, 512, 512])
            ...
            for img in imgs:
                ...
                res.append(img)
            res = torch.stack(res, dim=0) #torch.Size([5, 3, 512, 512])
            packed_results['inputs'] = res
