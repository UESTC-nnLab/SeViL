...
class Resize(MMCV_Resize):
    ...
    def _resize_bboxes(self, results: dict) -> None:
        ...
        if results.get('gt_bboxes', None) is not None:
            results['gt_bboxes'].rescale_(results['scale_factor']) ####*** KeyError: 'scale_factor'
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.scale:
            ...
            #reresults['img']有可能是一个list，也有可能不是
        ...
        for i in range(len(results['img'])):
            res['img'] = results['img'][i].copy()  #(512,512,3)
            self._resize_img(res)
            if i == 0:
                self._resize_bboxes(res)
                self._resize_masks(res)
                self._resize_seg(res)
                self._record_homography_matrix(res)
                for key in res.keys():
                    if key not in results.keys():
                        results[key] = res[key]
                    if key != 'img':
                        results[key] = res[key]
            results['img'][i] = res['img']
            results['img_shape'] = res['img_shape']
        return results
