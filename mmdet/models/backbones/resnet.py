...
class ResNetSeq(BaseModule):
    ...
    def __init__(self,
        ...
        #####TextEnhance
        # self.text_enhance = ClipTextEncoder()
    ...
    def forward(self, x):
        ...
        for i in range(frame):
            x = xcopy[:,i,:,:,:]
            # #####文本增强 ###### divide patch 训练时间太长了
            # x = self.text_enhance(x)
            # #####
            if self.deep_stem:
                x = self.stem(x)#
            else:
...
class ClipTextEncoder(nn.Module):
    ...
    Clip TextEncoder
    """
    def __init__(self, type = "ViT-B/32", patch_num = 16):
        super().__init__()
    ...
    # "The target is often specific types of objects, such as heat sources, aircraft, vehicles, etc., that contrast with the background and are usually parts of interest in the scene."
        ...
        self.type = type
        self.model, self.preprocess = clip.load(self.type, device=self.device)  # 'RN50', 'RN101', 'RN50x4', 'RN50x16', 'RN50x64', 'ViT-B/32', 'ViT-B/16', 'ViT-L/14', 'ViT-L/14@336px'
        
    def forward(self, x):
        # pdb.set_trace()
        # print(x.shape)
        text_features = self.model.encode_text(self.text) # [10, 512]
        h, w = x.shape[-2:] # 512
        grid_h, grid_w = h//self.patch_num, w//self.patch_num
        # 存储每个区域与文本描述的相似度
        similarity_scores = []
        for i in range(self.patch_num):
            for j in range(self.patch_num):
                x1, y1 = i * grid_h, j * grid_h
                x2, y2 = (i + 1) * grid_h, (j + 1) * grid_h
                 # 裁剪图像区域
                patch = x[:, :, y1:y2, x1:x2]  # (batch_size, channels, height, width)
                patch = F.interpolate(patch, (224, 224), mode='bilinear')
                image_features = self.model.encode_image(patch) # [1, 512]
                # print('-----image_features-----')
                # print(image_features.shape)
                extended_text_features = text_features.unsqueeze(0).repeat(image_features.shape[0], 1, 1) #[3,10,512]
                extended_patch_features = image_features.unsqueeze(1).repeat(1, text_features.shape[0], 1) # [3,10,512]
                similarity = F.cosine_similarity(extended_patch_features, extended_text_features,dim=-1)
                similarity_scores.append(similarity.detach().cpu().numpy())
        # 将相似度得分转换为 numpy 数组
        similarity_scores = np.array(similarity_scores).reshape(self.patch_num, self.patch_num, len(self.text)*x.shape[0])
        region_comprehensive_similarity = np.sum(similarity_scores, axis=-1)  # 对所有文本描述的相似度求和
        # print(region_comprehensive_similarity.shape)
        # print(region_comprehensive_similarity)
        new_tensor = torch.zeros_like(x)
        for i in range(self.patch_num):
            for j in range(self.patch_num):
                x1, y1 = i * grid_h, j * grid_h
                x2, y2 = (i + 1) * grid_h,(j + 1) * grid_h
                image_patch = x[:, :, y1:y2, x1:x2]
                # 获取当前区域的相似度分数
                region_similarity = region_comprehensive_similarity[i, j]
                # 获取最大相似度文本的权重（或者使用综合相似度的总和）
                weight = region_similarity * 100
                # 加权图像区域特征
                weighted_patch_features = image_patch * weight
                new_tensor[:, :, y1:y2, x1:x2] = weighted_patch_features
        return new_tensor