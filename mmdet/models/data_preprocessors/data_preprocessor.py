...
class MultiBranchDataPreprocessor(BaseDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        if training is False:
            ...
            sub_tensors = [tensor[i] for i in range(tensor.size(0))]
            data['inputs'] = sub_tensors
            temp = self.data_preprocessor(data, training)
            res.append(temp['inputs'])
            res2 = torch.stack(res, dim=0)
            temp['inputs'] = res2
            return temp
        ...
        for branch, _data in multi_branch_data.items():
            # pdb.set_trace()
            length = len(_data['inputs'])
            res = []
            samples = []
            temp = _data.copy()
            for i in range(length):
                tensor = temp['inputs'][i]
                sub_tensors = [tensor[i] for i in range(tensor.size(0))]
                _data['inputs'] = sub_tensors
                newlist = []
                newlist.append(temp['data_samples'][i])
                _data['data_samples'] = newlist
                multi_branch_data[branch] = self.data_preprocessor(_data, training) #调用的是DetDataPreprocessor的forward
                res.append(multi_branch_data[branch]['inputs']) # 
                samples.append(multi_branch_data[branch]['data_samples'][0])
            res = torch.stack(res, dim=0)
            multi_branch_data[branch]['inputs'] = res
            multi_branch_data[branch]['data_samples'] = samples
            # length = _data['inputs'][0].shape[0]
            # temp = []
            # for i in range(length):
            #      datacopy = datacopy1.copy()
            #      print(datacopy['inputs'][0].shape)
            #      x = _data['inputs'][0]
            #      datacopy['inputs'][0] = x[i,:,:,:]
            #      t = self.data_preprocessor(datacopy, training)['inputs']
            #      temp.append(t)
            # temp = temp[:,:,-1]
            # multi_branch_data[branch] = torch.stack(temp, dim=0)
